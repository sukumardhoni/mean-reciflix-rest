...
exports.create = function (req, res) {
	...
	console.log('new recipe from client' + JSON.stringify(req.body.videoId));
	var videoKey = req.body.videoId;
	Vrecipe.find({
		videoId: videoKey
		...
		// 	console.log('err', err)
		// } else {
		// 	console.log('all recipes from db' + JSON.stringify(allRecipes));
		// }
      console.log('all recipes from db');
		// if (allRecipes.length == 0) {
    //    console.log('new recepi saved called oncce ');
    ...
    //   });
		// } else {
    //   var errResp = {
    //       msg: 'recipe is already saved with videoId ',
    //       alreadySavedVideoId:videoKey
    //     };
    //     console.log('already saved '+ JSON.stringify(errResp.msg))
		// 		res.json(errResp)
		// }

		/* for (var i = 0; i < allRecipes.length; i++) {
			if (req.body.videoId == allRecipes[i].videoId) {
			...
			//console.log('all vedioIds '+JSON.stringify(allRecipes[i].videoId));
		} */
	})
	...
	Subscription.find().exec(function (err, subscribers) {
		if (err) {
			...
			for (var k = 0; k < subscribers.length; k++) {
				...
				var vapidKeys = {
					publicKey: 'BCZLs69d4JA_FD9LyXugozgVdBCL1vHwKEBVlaW1M46fLIJ64tC2DWZ33xxr3t5uO8jEPrJCND4Q8LEOFQnxo0g',
					privateKey: 'MAtSqNocVByqCleEW9LArR0CMTPemrFL-ZhvDVuq0e4'
				};
...
exports.getRecipesByFilterSubCats = function (req, res) {
	var deviceInfo = req.headers.device;
	var emailInfo = req.headers.email;
...
exports.getRecipesByFilterCats = function (req, res) {


	console.log("@@@@@@ PARAMS ID : " + JSON.stringify(req.params.newCatId))
	console.log("@@@@@@ PAGE ID : " + JSON.stringify(req.params.pageId))
	var deviceInfo = req.headers.device;
	var emailInfo = req.headers.email;
