...
exports.getCatsVthSubcats = function (req, res) {
	...
	Category.find(mongoQuery).exec(function (err, cats) {
		if (err) {
			...
			// console.log("ALL cats : " + JSON.stringify(cats))
			var forloopCount = 0;
			var afterResultCount = 0;
			for (var i = 0; i < cats.length; i++) {
				// forloopCount = forloopCount + 1;
				// console.log("forloopCount : " + JSON.stringify(forloopCount))
				var catId = cats[i].catId;
				catsArray.push(getSubCats(catId, cats[i].displayName));
			}
...
function getSubCats(catId, catDisplayName) {
		...
		SubCats.find({
			...
			if (err) {
				...
				for (var i = 0; i < subCategories.length; i++) {
					var subCatObj = {
						subCatId: subCategories[i].subCatId,
						subCatDisplayName: subCategories[i].displayName,
						catId: subCategories[i].catId
					}
